package com.deepblue.dab.board.controller;
...
import com.deepblue.dab.board.model.service.BoardService;
...
public class BoardController {
	...
	public ModelAndView boardDetailMethod(ModelAndView mv, 
		...
		//댓글 리스트 조회 
		ArrayList<Board> list = boardService.selectReply(board.getBoard_ref());
	...
	public String replyInsertMethod(Board reply, 
		...
		//해당 댓글에 대한 원글 조회
		Board origin = boardService.selectBoard(
				reply.getBoard_ref());
		...
		if(boardService.insertReply(reply) > 0) {
			...
			model.addAttribute("message", 
				reply.getBoard_ref() + "번 글에 대한 댓글 등록 실패");
			return "common/error";
		}
	...
	public String boardUpdateMethod(
		...
		if(boardService.updateOrigin(board) > 0) {
			...
			model.addAttribute("message", 
					board.getBoard_num() + "번 게시 원글 수정 실패!");
			return "common/error";
		}
	}
	...
	public String replyUpdateMethod(Board reply, 
		...
		if(boardService.updateReply(reply) > 0) {
			...
			model.addAttribute("message", 
					reply.getBoard_num() + "번 글 수정 실패");
			return "common/error";
		}
	}
	
	//공지글 제목 검색용
		@RequestMapping(value="searchTitle.do", method=RequestMethod.GET)
		...
		//공지글 작성자 검색용
		@RequestMapping(value="searchWriter.do", method=RequestMethod.GET)
		...
		public String boardSearchDateMethod(SearchDate date, 
				@RequestParam(name="page", required=false) String page,
				Model model) {
			int currentPage = 1;
			if(page != null) {
			...
			int limit = 10;  //한 페이지에 출력할 목록 갯수
			//전체 페이지 갯수 계산을 위해 총 목록 갯수 조회해 옴
			int listCount = boardService.selectSearchDListCount(date);
			//페이지 수 계산
			//주의 : 목록이 11개이면 페이지 수는 2페이지가 됨
			// 나머지 목록 1개도 1페이지가 필요함
			int maxPage = (int)((double)listCount / limit + 0.9);
			...
			ArrayList<Board> list = boardService.selectSearchDate(searchpaging);
			...
			if(list.size() > 0) {
				...
				model.addAttribute("message", "해당 날짜에 등록된 게시글 정보가 없습니다.");
				return "common/error";
			}
		}
